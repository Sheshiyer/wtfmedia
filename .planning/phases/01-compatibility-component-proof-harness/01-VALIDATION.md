...
| 01-07-binding | 01-07 | 7 | QUAL-06 budget authority binding | — | Approved budgets are a durable prerequisite: later tests load only the approved artifact and reject hash drift or missing metrics | manual + automated | Receipt schema_version 2 with seven dimension groups (home/episodes/connections/chat/bundle/interactions/rag), SHA-256-bound to baseline+proposal, owner ref embedded; T-01-22 verifies VALIDATION.md carries plan id, owner ref, and T-01-21 while flags stay false | ✅ | ✅ closed 2026-08-22 |
| 01-21-T1/T2/T3 | 01-21 | 8 | QUAL-06 | T-01-65, T-01-66, T-01-67 | Complete legacy presentation boundary is frozen and owner-approved before any token/shell/route mutation | dependency graph + visual oracle + manual gate | `cd web && node scripts/capture-legacy-presentation.mjs --check && npm run test:browser -- tests/rollback/legacy-presentation.spec.ts` (with `PORT=4173` while a foreign process holds :3000) | ✅ 20-node graph hash-bound; 12 captures at 320/768/1440 hash-bound in `web/tests/visual/legacy/phase1-legacy-baseline.manifest.json` | ✅ approved 2026-08-22 (`owner-explicit-approval-2026-08-22-legacy-presentation-baseline-via-askuserquestion-review`) |
...
## Plan 01-21 Legacy Presentation Baseline Authority (Wave 8)

Plan `01-21` freezes the complete legacy presentation boundary as the owner-approved rollback oracle that Plans 01-08 and 01-10 must preserve or restore. Task 1 captured the 20-node repository-owned dependency graph (`web/tests/contracts/legacy-presentation-dependencies.json`, graph digest `891745a8d370994f…`, hash-bound to the compatibility manifest `91af632a…`) via `web/scripts/capture-legacy-presentation.mjs`. Task 2 produced the dual-mode oracle `web/tests/rollback/legacy-presentation.spec.ts` with twelve hash-bound ready-state captures at 320/768/1440 across `/`, `/episodes`, `/connections`, `/chat` (canvas masked on `/connections` for determinism, CSS animations disabled, loopback-only network) plus live focus, reduced-motion, and chat safe-error probes. Task 3's blocking human checkpoint was satisfied by an explicit AskUserQuestion owner review on 2026-08-22; the manifest in `web/tests/visual/legacy/phase1-legacy-baseline.manifest.json` is now `approved` with the owner reference recorded and all hashes, browser identity (Chromium 1234 / 151.0.7922.34, Playwright 1.62.1), and coverage intact.

**Approval reference**: `owner-explicit-approval-2026-08-22-legacy-presentation-baseline-via-askuserquestion-review`

**Binding rule for later plans**: visual migration plans (01-08 token layer, 01-10 shell) must keep `capture-legacy-presentation.mjs --check` green (any presentation-file mutation requires re-capture under a separately approved plan), keep every snapshot hash matching the approved manifest unless the manifest is re-captured and re-approved, and preserve the live behavior contract (keyboard-reachable navigation, chat safe-error surface) asserted by the rollback spec. Drift in any of these disables the migration run rather than being skipped.

**Threat evidence**: T-01-65 executed via `node web/scripts/run-phase1-threat.mjs --plan 01-21 --task 1` → `passed`; T-01-66 via `--plan 01-21 --task 2` → `passed` (with `PORT=4173` inherited by the runner-spawned webServer while a foreign process holds :3000); T-01-67 executes under Task 3 and verifies the manifest is approved with owner reference, graph digest, and full viewport coverage while the Nyquist flag remains false in frontmatter.
