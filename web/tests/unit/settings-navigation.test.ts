...
describe("nested settings navigation", () => {
  ...
  it("resolves only canonical nested settings paths", () => {
    expect(settingsSectionForPath("/beta/settings/workspace/analytics")?.id).toBe("analytics");
    expect(settingsSectionForPath("/beta/settings")).toBeNull();
    expect(settingsSectionForPath("/beta/settings/workspace/analytics/extra")).toBeNull();
  });
});
