...
export function MemberSessionNavigator({ activeConversationId, onNavigate, refreshKey = 0 }: { activeConversationId?: string; onNavigate?: () => void; refreshKey?: number }) {
  ...
  return (
    <nav aria-label="Your conversations" className="grid gap-3" data-member-session-navigator>
      ...
      {state === "ready" && history ? <div className="grid gap-2">{history.conversations.map((conversation) => {
        const href = memberConversationHref(conversation.id);
        return href ? <Link key={conversation.id} href={href} onClick={onNavigate} aria-current={conversation.id === activeConversationId ? "page" : undefined} className={`block border-2 p-3 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-information ${conversation.id === activeConversationId ? "border-information bg-information/15" : "border-foreground/20 bg-canvas hover:border-foreground"}`}><p className="truncate font-body text-sm font-semibold text-foreground">{conversation.title}</p><p className="mt-1 font-label text-[10px] uppercase tracking-wide text-muted">{conversation.sourceMode} evidence</p></Link> : null;
