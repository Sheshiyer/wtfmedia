...
updated: 2026-09-12T05:00:00+05:30
---
...
- [DEFERRED-VERIFY] ISC-321: The accepted Alpha-authority/Beta-overlay staging candidate completes a documented rollback rehearsal that restores the prior staging Edge version and re-verifies health and owner-scoped history (probe: named-profile rollback receipt; follow-up `WTF-BETA-ROLLBACK-01`).
- [ ] ISC-322: The owner reconciles historical remote tag `v0.3.3-beta.2` before any later Beta tag is published; the historical tag is not deleted, moved, reused, or silently reinterpreted (probe: remote tag inventory plus recorded release decision).
...
| ISC-252..326 | Beta 0.1 release evidence and promotion gate | immutable local merge identity, Alpha-only evidence authority, Beta identity/session overlay, canonical grouped-citation parsing, sanitized service-binding transport, exact-candidate staging receipts, remote delivery topology, and explicit non-goals | source evidence stays source-only; each live claim has its exact Git, Cloudflare, D1, or authenticated-IAB receipt | Git + precedence/checklist review + owner-authorized GitHub/Cloudflare/Clerk/D1/IAB receipts |
...
- ISC-320 through ISC-322 deferred verification: only the existing super-admin has completed the current live pass. Signed-out and remaining-role isolation, a staging rollback rehearsal, and historical `v0.3.3-beta.2` tag reconciliation remain required before Beta promotion.
