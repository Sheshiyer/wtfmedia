...
- [x] ISC-291: Member and operator principals render one accepted Alpha-derived Beta chat presentation through typed adapters while `mcnv_*` and `cnv_*` stores, authorization, and lifecycle capabilities remain separate (probe: import graph, focused unit contracts, and authenticated super-admin IAB).
- [x] ISC-292: A verified Clerk identity/session resolves principal context once across same-session Beta route navigation, locally rechecks each destination, and does not repeat the full-screen workspace-opening state on every page (probe: principal runtime unit test and authenticated navigation observation).
- [x] ISC-293: Signed-in Beta navigation exposes no ingest destination or chat/settings bottom dock, and Settings omits Readiness, AI Route, YouTube Analytics, RAG & Sources, and duplicate Release Mutation without weakening direct-route policy enforcement (probe: navigation source contracts and authenticated IAB).
