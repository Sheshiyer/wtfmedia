...
- [ ] ISC-195: The ordinary-member Ask input reuses the accepted Alpha `AskComposer` behavior, including unpositioned capsule composition and overflow-aware parent placement (probe: historical source map, component contract, and browser test). Source/UI reuse is proven; complete viewport/interaction acceptance remains tied to ISC-223 and ISC-239.
...
- [x] ISC-201: Superseded acceptance language; the two-button icon-labelled Ask WTF/Settings pill is rejected and replaced by ISC-222 (probe: 2026-09-11 owner annotation). Owner annotation is complete; live replacement acceptance still drives via ISC-222.
...
- [ ] ISC-214: Long member threads own a bounded scroll region and do not force a reader who scrolled upward back to the newest answer (probe: component/browser scroll test). Full viewport and long-thread acceptance remains open in ISC-223 and ISC-239.
- [ ] ISC-215: Expanded source details and the final message remain reachable above the compact composer and member navigation pill (probe: 320x710, 768px, and 1382x887 browser viewports). Complete current-candidate viewport acceptance remains in ISC-299; ISC-239 is retained as historical convergence scope.
...
- [DEFERRED-VERIFY] ISC-238: Historical/superseded: Signed-out, Member A, Member B, editor, admin, super-admin, and suspended/revoked personas pass the complete staging route/screen/action/isolation matrix for the exact committed Beta candidate (probe: owner-authorized authenticated IAB matrix plus metadata-only D1 readback; follow-up `WTF-BETA-CONVERGENCE-LIVE-01`).
- [DEFERRED-VERIFY] ISC-239: Historical/superseded convergence scope: the member chat passes authenticated 1382x887, 1382x1180, and 320x710 layout, focus, drawer, composer, source-sheet, and long-thread acceptance for the exact committed Beta candidate (probe: owner-authorized authenticated IAB; current required gate is ISC-299; follow-up `WTF-BETA-CONVERGENCE-UX-01`).
...
- [x] ISC-247: Session cards expose Archive and a separate permanent Delete control in member adapters; local lifecycle contracts preserve saved preferences and prevent idempotency-key resurrection (probe: Edge lifecycle and browser unit tests). User-facing confirmation and anti-resurrection runtime proofs are tracked in ISC-225 through ISC-230.
...
- [DEFERRED-VERIFY] ISC-250: Historical/superseded convergence scope: an owner-authorized staging deployment/migration of one exact committed Beta candidate and a real Clerk/D1 Codex IAB persona matrix prove principal landing, route/screen/action isolation, and desktop/mobile acceptance before Beta is accepted (probe: current required gate is ISC-299 plus the Beta 0.1 checklist).
...
- [x] ISC-266: Staging has a pre-migration D1 Time Travel recovery bookmark and inventory plus a post-apply receipt proving reviewed migrations `0012`–`0015` applied with none remaining (probe: 2026-09-11 named-profile staging receipts in the Beta 0.1 checklist).
- [x] ISC-267: Candidate `abb5413` is deployed as Edge version `3b4aa27f-6c62-4077-a432-897f10002039` and web version `00cfe1b4-161e-4c86-addf-dd3894085d3d`; Wrangler proves canonical Clerk/D1 staging bindings, `ASSETS`, and only the paired staging service binding (probe: 2026-09-11 deployment/config/health receipts in the Beta 0.1 checklist).
...
- [ ] ISC-283: [DROPPED — the tag gate remains open but is refreshed without the obsolete next-tag assumption at ISC-322; see Decision 2026-09-12.]
...
- [x] ISC-289: Live staging evidence distinguishes working owner-scoped `cnv_*` persistence from grounded inference readiness: saved conversations reopen while the isolated staging R2/KV/Vectorize plane reports zero records (probe: canonical super-admin IAB plus named-profile aggregate resource counts).
- [x] ISC-290: Anti: the lineage correction performs no UI fix, build, commit, push, deployment, tag mutation, staging data copy, queue action, Clerk/DNS change, or production mutation (probe: Git status, command ledger, and remote deployment history boundary).
- [x] ISC-291: Member and operator principals render one accepted Alpha-derived Beta chat presentation through typed adapters while `mcnv_*` and `cnv_*` stores, authorization, and lifecycle capabilities remain separate (probe: import graph, focused unit contracts, and authenticated super-admin IAB). Multi-persona runtime acceptance now points to pending ISC-299; ISC-238 remains a historical predecessor.
- [x] ISC-292: A verified Clerk identity/session resolves principal context once across same-session Beta route navigation, locally rechecks each destination, and does not repeat the full-screen workspace-opening state on every page (probe: principal runtime unit test and authenticated navigation observation). Persona breadth is currently represented by pending ISC-299; ISC-238 is historical.
- [x] ISC-293: Signed-in Beta navigation exposes no ingest destination or chat/settings bottom dock, and Settings omits Readiness, AI Route, YouTube Analytics, RAG & Sources, and duplicate Release Mutation without weakening direct-route policy enforcement (probe: navigation source contracts and authenticated IAB). Navigation source contracts are verified; live viewport/multi-persona completion is pending at ISC-299, with ISC-238 retained as historical reference.
- [x] ISC-294: The admin roster is loaded from the authenticated D1-backed edge API through the paired web service binding and contains no prefabricated source data (probe: route source contract and authenticated super-admin IAB).
- [x] ISC-295: Exact UI/runtime candidate `32ec582d9ab953413710832c62e773eb212c00e1` passes Edge 380/380, web unit 205/205, web contracts 96/96, TypeScript, lint, OpenNext build, privacy with zero violations across 399 files, architecture freshness across 623 inputs after the release-record update, and `git diff --check` (probe: local command receipt).
...
- [ ] ISC-299: [DROPPED — the full persona gate remains open but exact candidate `25019a3` is superseded by the Alpha-authority/Beta-overlay runtime and ISC-320; see Decision 2026-09-12.]
...
- [ ] ISC-308: [DROPPED — superseded by the owner decision on 2026-09-12 that Beta must not own a separate staging corpus or ingest plane; see ISC-309 through ISC-322.]
- [x] ISC-309: Alpha remains the sole authority for corpus, ingest, retrieval, inference, grounding, citations, timestamps, and rich moments; Beta consumes that authority without cloning its data plane (probe: source ownership map, staging binding inventory, and live Alpha-backed answer receipt).
- [x] ISC-310: Beta owns only Clerk identity, D1 admission and RBAC, admin projections, and user-scoped conversation, message, history, and lifecycle persistence (probe: route-policy map, D1 schema inventory, and authenticated staging IAB).
- [x] ISC-311: The staging Alpha gateway uses a reviewed Cloudflare service binding, forwards only a bounded prior-turn envelope, and forwards no Clerk credential, D1 owner identifier, saved memory, secret, or transcript text (probe: gateway unit tests and source inspection).
- [x] ISC-312: Beta validates Alpha response state and citation headers before persisting a sanitized answer projection; the D1 envelope preserves the reviewed rich source metadata without storing transcript excerpts or private transport fields (probe: Alpha-gateway and chat-history persistence tests).
- [x] ISC-313: Staging Edge resource bindings are only isolated staging D1 plus `WTFMEDIA_ALPHA_WEB -> wtfmedia-web` and include no AI, Vectorize, R2, KV, queue, or ingest resource; version readback still lists a legacy inert `INGEST_TOKEN` secret, but `/v1/admin/enqueue` is disabled and no queue/resource can consume it, while removal requires a separate owner-approved credential task (probe: named-profile Wrangler version/binding readback and route contracts).
- [x] ISC-314: Staging D1 has pre-migration Time Travel bookmark `0000006b-00000000-000050e3-0bb7072294902e18dbbd945231ef35dc`, reviewed migration `0016` applied, and no pending migration through `0016` (probe: named-profile D1 bookmark, apply, and migration-list receipts).
- [x] ISC-315: Exact runtime candidate `5229f99d0088987dabf6d7af15ca32f1cb9c6cec` is deployed only as staging Edge version `91b2fc7a-f657-4dd1-a66d-4f3c50ad7b21`, while staging web remains version `cbc25bba-c2a1-436f-8612-11813dbd3946` (probe: Git object identity and named-profile deployment readback).
- [x] ISC-316: Authenticated super-admin IAB creates and reloads a fresh grounded conversation with one cited episode and twenty persisted rich moments, without recording its machine/session identifier in repository evidence (probe: canonical staging IAB plus privacy-bounded conversation API readback).
- [x] ISC-317: The freshly grounded citation sheet remains usable with zero document-width overflow at desktop 1382x887 and mobile 320x710 (probe: authenticated IAB viewport and layout measurements).
- [x] ISC-318: The authenticated super-admin opens the live Admin Users surface backed by the staging D1 roster (probe: canonical staging IAB and admin-users API response).
- [x] ISC-319: Anti: this Alpha-authority correction changes no production Worker version, production data, corpus, ingest, queue, DNS, Clerk configuration, tag, PR #48, or PR #77 (probe: production Edge `ccd1d952-5be7-41c9-9275-f9d6b3b470a7`, production web `3d5a5965-14f3-486a-a608-330d539dec81`, GitHub readback, and command ledger).
- [DEFERRED-VERIFY] ISC-320: Signed-out, Member A, Member B, suspended/revoked, editor, admin, and super-admin principals complete the route, action, direct-URL, and cross-user isolation matrix against one exact staging candidate (probe: authenticated IAB matrix; follow-up `WTF-BETA-CONVERGENCE-LIVE-01`).
- [x] ISC-321: The accepted Alpha-authority/Beta-overlay staging candidate completes documented Edge and Web rollback/restore rehearsals with exact version IDs while staging health, public Alpha, and persistent data remain intact (probe: named-profile dry-run, deployment, health, and ingress receipts).
- [x] ISC-322: The owner reconciles historical remote tag `v0.3.3-beta.2` as immutable, historically inconsistent provenance rather than the current Beta or a deployment receipt; the tag is not deleted, moved, reused, or silently reinterpreted (probe: remote tag inventory, package-version readback, ancestry checks, and recorded release decision).
- [x] ISC-323: Exact post-QA candidate `748c0e4e8d732b5ecdbd3cda572c8bd67f2a32d6` passes all 387 Edge tests and is deployed only as staging Edge version `67e93676-8519-4e33-8b7b-83b45fa975a8`, while the staging web version remains unchanged (probe: exact Git/test receipt and named-profile deployment readback).
- [x] ISC-324: Beta reuses Alpha's canonical citation parser so a grouped marker such as `[1, 2]` resolves both cited indices and cannot turn an otherwise grounded answer into a false abstention presentation (probe: shared-parser source contract and grouped-citation regression tests).
- [x] ISC-325: A fresh authenticated staging answer and reload preserve rich sources while rendering zero false abstention labels after the grouped-citation parser repair (probe: privacy-bounded canonical IAB observation and persisted-answer readback).
- [x] ISC-326: The source-sheet label `N cited` reports the count of answer-cited excerpts, while the editor-sheet moment rows remain a distinct rich-moment projection and are not presented as the citation count (probe: source-sheet copy/data contract and authenticated IAB).
- [x] ISC-327: Beta exposes no ingest capability, navigation destination, protected route policy, or callable ingest UI, while Alpha `/ops/ingest` and its existing role grants remain intact (probe: route/policy/DTO source contracts, direct-URL IAB denial, and Alpha regression tests).
- [x] ISC-328: Exact source candidate `409a8ae205c515e453477703bc2342744231006d` passes Edge 388/388, web unit 225/225, web contracts 97/97, TypeScript, lint, OpenNext build with 89 pages, privacy with zero violations across 400 files, architecture freshness across 631 inputs, and `git diff --check` (probe: exact-candidate local command receipt).
- [x] ISC-329: Final staging runs Edge `71f8c529-1040-4248-8e36-351741fc4edb` and Web `b4e4ef19-2990-4f1b-bdf1-c9d8d8556792`; Edge is runtime-equivalent to candidate `409a8ae` from `2c04bc2` because later changes affect only Web logout and release evidence (probe: Git diff plus named-profile deployment readback).
- [x] ISC-330: Separately authorized versioned secret cleanup removes inert staging `INGEST_TOKEN`, and final secret-name readback exposes only Clerk, Edge transport, and origin-proof secrets (probe: Wrangler version inspection, deployment, and secret-name list without secret values).
- [x] ISC-331: Final staging health reports `service=wtfmedia-edge-staging`, `environment=staging`, `inferenceService=wtfmedia-web`, and `corpusAuthority=alpha_public_api`; `/v1/admin/enqueue` and `/beta/preview` remain absent (probe: exact endpoint responses).
- [x] ISC-332: The accepted Alpha-derived chat, grounded citations, rich source sheet, playback/export, owner history, archive actions, long-title clamping, drawer focus return, and composer remain usable without horizontal overflow at 1382x887, 1382x1180, and 320x710 (probe: authenticated canonical-host IAB and CDP layout measurements).
- [x] ISC-333: Real Clerk logout terminates the live session and redirects to `/sign-in`; direct navigation to the prior protected chat exposes no private conversation, answer, history, archive action, or chat composer (probe: final-candidate canonical-host IAB).
- [x] ISC-334: Actual authentication session and refresh cookies are host-only to `beta-staging.wtfhq.in`; parent-domain Clerk client metadata does not establish Beta session authority (probe: privacy-bounded IAB CDP cookie attributes without recording cookie values).
- [x] ISC-335: Operator history remains Archive-only for Beta 0.1; permanent Operator Delete is unavailable until a separately reviewed detach/tombstone, saved-memory, audit, and anti-resurrection contract exists (probe: D1 constraints, service source, lifecycle tests, and recorded owner decision).
- [x] ISC-336: Edge rollback moves accepted `67e93676-8519-4e33-8b7b-83b45fa975a8` to predecessor `91b2fc7a-f657-4dd1-a66d-4f3c50ad7b21` and restores it before the no-ingest/secret successor, with staging health and the owner-scoped application preserved (probe: Wrangler version deployment and health receipts).
...
| ISC-252..338 | Beta 0.1 release evidence and promotion gate | immutable local merge identity, Alpha-only evidence authority, Beta identity/session overlay, no duplicate ingest surface, canonical grouped-citation parsing, sanitized service-binding transport, exact-candidate staging and rollback receipts, real logout, remote delivery topology, and explicit non-goals | source evidence stays source-only; each live claim has its exact Git, Cloudflare, D1, or authenticated-IAB receipt | Git + precedence/checklist review + owner-authorized GitHub/Cloudflare/Clerk/D1/IAB receipts |
...
- 2026-09-12 04:51 IST: refined: the owner rejects a separate Beta corpus or ingest lane. Alpha is the sole corpus, ingest, retrieval, inference, grounding, citation, timestamp, and rich-moment authority. Beta is a narrow authenticated overlay for Clerk identity, D1 admission/RBAC/admin, and user-scoped sessions, messages, history, and lifecycle. ISC-308 is retained as a dropped tombstone; ISC-309 through ISC-322 bind the replacement architecture, exact staging receipts, live super-admin proof, no-production boundary, and still-open persona, rollback, and tag gates.
...
- 2026-09-12 | conjectured: preserving Alpha's answer and rich source envelope was sufficient for Beta to classify every grounded response correctly
  ...
  criterion now: ISC-323 through ISC-326 bind the final candidate, shared grouped-marker parsing, zero-abstention live reload, and explicit citation-versus-moment count semantics
- 2026-09-12 | conjectured: grounded Beta staging required copying or bootstrapping an approved public corpus into a separate Beta R2, KV, and Vectorize plane
  refuted by: the owner's explicit architecture correction and live Alpha service-binding proof show that Alpha already owns the accepted corpus, inference, grounding, citations, and rich moments while Beta needs only authenticated persistence and governance
  learned: Beta should persist a sanitized, bounded projection of Alpha's answer and rich citation metadata alongside owner-scoped history, without acquiring corpus, ingest, model, vector, object, KV, or queue ownership
  criterion now: ISC-308 is a stable dropped tombstone; ISC-309 through ISC-319 verify the replacement Alpha-authority/Beta-overlay topology and live staging evidence, while ISC-320 through ISC-322 retain the remaining release gates
...
- ISC-175 evidence: authenticated staging created a private conversation at the canonical `/beta/chat/<conversation>` route; two turns survived reload, and browser back/forward preserved the root/new-chat and selected-conversation states. The exact conversation identifier is intentionally omitted.
...
- ISC-291 through ISC-295 verification: candidate `32ec582` passes Edge 380/380, web unit 205/205, web contracts 96/96, typecheck, lint, OpenNext build with 89 pages, privacy 0/399, architecture freshness across 623 inputs after the release-record update, and `git diff --check`; focused source contracts cover adapters, admission, navigation, settings, and roster proxy behavior.
...
- ISC-298 through ISC-300 historical boundary: operator Delete stays absent because no operator tombstone contract exists; the full persona/viewport matrix remains open, while the former separate-staging-corpus gate is superseded by the owner decision recorded at ISC-308 through ISC-313; that earlier command/deployment scope changed no production, edge, data, auth, DNS, queue, secret, or public-Alpha state.
...
- ISC-306 through ISC-308 historical verification: exact runtime candidate `25019a3` passed the expanded 381/220/96 source suites and OpenNext build, then deployed only as staging Edge `a68b4793-ed57-4c1c-8436-2e574ccdb192` and web `cbc25bba-c2a1-436f-8612-11813dbd3946`. Authenticated IAB created and reloaded the truthful empty-corpus turn at all required viewport sizes with zero horizontal overflow. The later owner correction supersedes the then-pending bootstrap premise and retains ISC-308 as a dropped tombstone.
- ISC-309 through ISC-313 verification: source contracts and focused gateway tests make Alpha the single corpus/inference/citation authority, constrain Beta to authenticated D1-backed identity/session/admin state, bound prior-turn transport, reject sensitive-field forwarding, validate Alpha citation headers, and retain rich answer metadata within the reviewed persistence envelope. Named-profile staging resource readback exposes only `DB` and `WTFMEDIA_ALPHA_WEB -> wtfmedia-web`; it exposes no staging AI, Vectorize, R2, KV, queue, or ingest resource. Version readback still lists a legacy inert `INGEST_TOKEN` secret; the enqueue route is disabled and secret removal remains separately owner-gated.
- ISC-314 evidence: before applying reviewed staging migration `0016`, D1 Time Travel returned bookmark `0000006b-00000000-000050e3-0bb7072294902e18dbbd945231ef35dc`; post-apply migration inventory reports no pending migration through `0016`.
- ISC-315 evidence: Git resolves exact runtime candidate `5229f99d0088987dabf6d7af15ca32f1cb9c6cec`; named-profile readback identifies staging Edge version `91b2fc7a-f657-4dd1-a66d-4f3c50ad7b21` and unchanged staging web version `cbc25bba-c2a1-436f-8612-11813dbd3946`.
- ISC-316 through ISC-318 evidence: authenticated canonical staging IAB as the existing super-admin created and reloaded a fresh privacy-bounded conversation; the grounded source sheet reports one cited episode and twenty rich moments, remains within document width at 1382x887 and 320x710, and the live Admin Users surface loads the D1-backed roster. No exact conversation or session identifier is retained in this artifact.
- ISC-319 evidence: named-profile production readback remains Edge `ccd1d952-5be7-41c9-9275-f9d6b3b470a7` and web `3d5a5965-14f3-486a-a608-330d539dec81`; the command/Git ledger contains no production, corpus, ingest, queue, DNS, Clerk, tag, PR #48, or PR #77 mutation.
- ISC-320 through ISC-322 verification: only the existing super-admin has completed the authenticated live pass, so Member A/B, editor, admin, and suspended/revoked isolation remain deferred. Edge and Web rollback/restore rehearsals are complete. The owner reconciled `v0.3.3-beta.2` as immutable historical inconsistency rather than a current release or deployment receipt.
- ISC-323 evidence: Git resolves exact post-QA candidate `748c0e4e8d732b5ecdbd3cda572c8bd67f2a32d6`; Edge tests pass 387/387; named-profile readback identifies staging Edge version `67e93676-8519-4e33-8b7b-83b45fa975a8`, with the staging web version unchanged from the prior receipt.
- ISC-324 through ISC-326 evidence: regression tests exercise Alpha's canonical parser against grouped marker `[1, 2]`; a fresh authenticated staging answer and reload show zero abstention label while rich sources persist. The UI contract reports `N cited` from answer-cited excerpts and renders editor-sheet moment rows as a separate projection rather than inflating the citation count.
- ISC-327 through ISC-331 evidence: final source/tests remove the Beta ingest surface without altering Alpha; named-profile readback pins Edge `71f8c529-1040-4248-8e36-351741fc4edb` and Web `b4e4ef19-2990-4f1b-bdf1-c9d8d8556792`; the staging secret-name list omits `INGEST_TOKEN`; health preserves `alpha_public_api`; disabled endpoints remain absent.
- ISC-332 through ISC-335 evidence: authenticated IAB and CDP prove the Alpha-derived rich chat at all three required viewports, host-only session/refresh cookies, and real Clerk logout followed by signed-out denial. Source/schema review records the Beta 0.1 Archive-only operator lifecycle decision.
- ISC-336 through ISC-338 evidence: Wrangler moved Edge and Web to exact predecessors and restored the accepted versions at 100%. Health, signed-out ingress, and public Alpha remained available; production version readback, D1 aggregates, Git refs, and the command ledger show no production, corpus, D1-row, DNS, Clerk-config, tag, PR, or `beta_0.1` mutation.
