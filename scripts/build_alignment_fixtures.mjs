...
export function convertCoordinateEngine(intervals, fromSystem, timeSec, uncutMax, pubMax) {
...
export function generateBenchmarkData() {
  const completeEpisodes = rawEpisodes.map((ep) => {
    ...
    evalCoordinates.push({
      source_timeline: "uncut",
      time_sec: 0.0,
      expected_target_time_sec: convertCoordinateEngine(ep.intervals, "uncut", 0.0, ep.uncut_duration_sec, ep.published_duration_sec).target_time_sec,
      expected_status: convertCoordinateEngine(ep.intervals, "uncut", 0.0, ep.uncut_duration_sec, ep.published_duration_sec).status,
      description: "Exact zero boundary on uncut timeline"
    });
    evalCoordinates.push({
      source_timeline: "published",
      time_sec: 0.0,
      expected_target_time_sec: convertCoordinateEngine(ep.intervals, "published", 0.0, ep.uncut_duration_sec, ep.published_duration_sec).target_time_sec,
      expected_status: convertCoordinateEngine(ep.intervals, "published", 0.0, ep.uncut_duration_sec, ep.published_duration_sec).status,
      description: "Exact zero boundary on published timeline"
    });
    evalCoordinates.push({
      source_timeline: "uncut",
      time_sec: ep.uncut_duration_sec,
      expected_target_time_sec: convertCoordinateEngine(ep.intervals, "uncut", ep.uncut_duration_sec, ep.uncut_duration_sec, ep.published_duration_sec).target_time_sec,
      expected_status: convertCoordinateEngine(ep.intervals, "uncut", ep.uncut_duration_sec, ep.uncut_duration_sec, ep.published_duration_sec).status,
      description: "Exact end boundary on uncut timeline"
    });
    evalCoordinates.push({
      source_timeline: "published",
      time_sec: ep.published_duration_sec,
      expected_target_time_sec: convertCoordinateEngine(ep.intervals, "published", ep.published_duration_sec, ep.uncut_duration_sec, ep.published_duration_sec).target_time_sec,
      expected_status: convertCoordinateEngine(ep.intervals, "published", ep.published_duration_sec, ep.uncut_duration_sec, ep.published_duration_sec).status,
    ...
    for (const interval of ep.intervals) {
      if (interval.interval_status === "cut_from_published") {
        ...
        const resStart = convertCoordinateEngine(ep.intervals, "uncut", interval.uncut_start_sec, ep.uncut_duration_sec, ep.published_duration_sec);
        evalCoordinates.push({
          source_timeline: "uncut",
        ...
        const resEnd = convertCoordinateEngine(ep.intervals, "uncut", interval.uncut_end_sec, ep.uncut_duration_sec, ep.published_duration_sec);
        evalCoordinates.push({
          source_timeline: "uncut",
    ...
    while (evalCoordinates.length < 100) {
      ...
      if (isUncut) {
        ...
        const res = convertCoordinateEngine(ep.intervals, "uncut", roundedT, ep.uncut_duration_sec, ep.published_duration_sec);
        evalCoordinates.push({
          source_timeline: "uncut",
          time_sec: roundedT,
        ...
        const roundedT = Math.round(t * 100) / 100;
        const res = convertCoordinateEngine(ep.intervals, "published", roundedT, ep.uncut_duration_sec, ep.published_duration_sec);
        evalCoordinates.push({
          source_timeline: "published",
          time_sec: roundedT,
          expected_target_time_sec: res.target_time_sec,
...
export function ensureFixtureFile(destPath) {
