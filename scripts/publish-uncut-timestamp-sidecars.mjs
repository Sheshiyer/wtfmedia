...
function uploadSidecar(tempRoot, entry) {
  ...
  if (result.status !== 0) throw new Error(`r2_upload_failed:${entry.videoId}`);
}
...
async function main() {
  ...
  const selectedMapPath = resolve(argument(args, "--map", mapPath));
  const selectedJobsPath = resolve(argument(args, "--jobs", jobsPath));
  const map = JSON.parse(readFileSync(selectedMapPath, "utf8"));
  const jobs = JSON.parse(readFileSync(selectedJobsPath, "utf8"));
  const bundle = buildUncutTimestampBundle({ map, jobs });
  const sidecarEntries = bundle.entries.filter((entry) => entry.job.timestampsKey);
  const metadataOnlyEntries = bundle.entries.filter((entry) => !entry.job.timestampsKey);

  if (!apply) {
    console.log(JSON.stringify({
      mode: "dry_run",
      jobs: bundle.entries.length,
      sidecars: sidecarEntries.length,
      metadataRefreshes: metadataOnlyEntries.length,
      intervals: bundle.entries.reduce((sum, entry) => sum + entry.intervalCount, 0),
  ...
  try {
    if (enqueueOnly) {
      ...
      for (let index = 0; index < sidecarEntries.length; index += 1) {
        console.log(`Uploading timestamp sidecar ${index + 1}/${sidecarEntries.length}...`);
        uploadSidecar(tempRoot, sidecarEntries[index]);
      }
    }
    console.log(`${enqueueOnly ? "Using" : "Uploaded"} ${sidecarEntries.length} timestamp sidecars; enqueueing ${bundle.entries.length} uncut jobs...`);
    ...
    console.log(JSON.stringify({
      mode: "apply",
      uploaded: sidecarEntries.length,
      metadataRefreshes: metadataOnlyEntries.length,
      queued: receipt.queued ?? null,
      skipped: bundle.skipped.length,
    }));
  } finally {
