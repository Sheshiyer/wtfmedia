...
export function normalizeInterval(
  ...
  const uncutStart = Number(raw.uncutStartSec ?? raw.uncut_start_sec ?? 0);
  const uncutEnd = Number(raw.uncutEndSec ?? raw.uncut_end_sec ?? uncutStart);
  const pubStart = Number(raw.pubStartSec ?? raw.pub_start_sec ?? 0);
  const pubEnd = Number(raw.pubEndSec ?? raw.pub_end_sec ?? pubStart);
  const intervalIndex = Number(raw.intervalIndex ?? raw.interval_index ?? fallbackIndex);
  const status: IntervalStatus = raw.status ?? raw.interval_status ?? 'matched';
  const confidence = Math.max(0, Math.min(1, Number(raw.confidence ?? 1.0)));

  return {
    intervalIndex,
    uncutStartSec: Math.min(uncutStart, uncutEnd),
    uncutEndSec: Math.max(uncutStart, uncutEnd),
    pubStartSec: Math.min(pubStart, pubEnd),
    pubEndSec: Math.max(pubStart, pubEnd),
    status,
    confidence,
    note: raw.note,
  };
...
export function parseAndValidateIntervals(
  ...
  return normalized.map((item, idx) => ({
    ...item,
    intervalIndex: item.intervalIndex >= 0 ? item.intervalIndex : idx,
  }));
}
      ...
      if (timeSec < start - epsilon) {
        ...
        if (mid + 1 < intervals.length) {
          ...
          if (timeSec >= nextStart - epsilon && timeSec <= nextStart + epsilon) {
            if (next.status === 'matched' && interval.status !== 'matched') {
              return next;
            }
          }
        }
