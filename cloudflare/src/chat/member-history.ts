...
export type MemberConversationView = { conversation: MemberConversation; messages: MemberMessage[]; retryable?: true; retrySourceMode?: SourceMode; resumeMessageId?: string };
...
export async function getMemberConversation(db: DB, memberId: number, conversationId: unknown): Promise<MemberConversationView | null> {
  ...
  const messages = await db.prepare("SELECT id, conversation_id, sequence, role, content, source_metadata_json, grounding_state, model, model_fallback, request_id, idempotency_key, created_at FROM member_chat_messages WHERE conversation_id = ? ORDER BY sequence ASC").bind(conversationId).all<MemberMessage>();
  ...
  if (conversation.lifecycle_state === "active" && lastMessage?.role === "user") {
    ...
    return { conversation, messages: messages.results, retryable: true, retrySourceMode: parseSourceMode(scope?.sourceMode ?? conversation.source_mode), resumeMessageId: lastMessage.id };
  }
...
export async function prepareMemberTurn(db: DB, memberId: number, conversationId: string | undefined, input: MemberTurnInput, now = new Date().toISOString()): Promise<MemberTurn | null> {
  ...
  view = await getMemberConversation(db, memberId, conversationId);
