 ...
 * Uncut citations carry the approved Frame.io episode URL when one was
 * persisted, otherwise a direct catalogue ref (`uncut:{id}`).
 */
...
function uncutSourceIdentity(metadata: Record<string, unknown>, publicVideoId: string): string {
  ...
  return match && match[1] !== publicVideoId ? match[1] : publicVideoId;
}
...
export function resolveRequestedSources(
  ...
  if (requested !== "uncut") {
    ...
    const interleaved: DualSourceCitation[] = [];
    for (let index = 0; index < Math.max(uncutHits.length, publishedHits.length); index += 1) {
      if (uncutHits[index]) interleaved.push(uncutHits[index]);
      if (publishedHits[index]) interleaved.push(publishedHits[index]);
    }
    const usedSegments = new Set<string>();
    const citations = interleaved
      .filter((citation) => {
